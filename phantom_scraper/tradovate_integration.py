...
# ============================================================================
# REAL-TIME USER SYNC WEBSOCKET (Added Jan 2026)
# Separate WebSocket for real-time position/order/balance updates
# This reduces REST API polling and provides instant updates to hero cards
# ============================================================================

class TradovateUserSyncWebSocket:
    """
    WebSocket for real-time user data (positions, orders, balances).
    
    This is SEPARATE from the order WebSocket in TradovateIntegration.
    - Order WebSocket: For placing/modifying orders quickly
    - User Sync WebSocket: For receiving real-time position/PnL updates
    
    Uses Tradovate's user/syncrequest to subscribe to account changes.
    """
    
    def __init__(self, access_token: str, user_id: int, is_demo: bool = True):
        """
        Initialize User Sync WebSocket.
        
        Args:
            access_token: Valid Tradovate access token
            user_id: Tradovate user ID to subscribe to
        ...
        self.position_callback = None  # Called on position updates
        self.order_callback = None     # Called on order updates
        self.balance_callback = None   # Called on balance updates
        self.error_callback = None     # Called on errors
        
        # Cache for latest data
        self.positions_cache: Dict[int, Dict] = {}  # position_id -> position data
        self.last_update_time: float = 0
    
    def _next_message_id(self) -> int:
        """Get next message ID for WebSocket requests."""
        self._message_id += 1
    ...
    async def connect(self) -> bool:
        ...
        """
        if not WEBSOCKETS_AVAILABLE:
            logger.error("websockets library not available for UserSyncWebSocket")
            return False
        
        try:
            logger.info(f"🔌 UserSync: Connecting to {self.ws_url}")
            ...
            self.connected = True
            logger.info("✅ UserSync: WebSocket connected")
            return True
        except Exception as e:
            logger.error(f"❌ UserSync: Connection failed: {e}")
            self.connected = False
            return False
    
    async def authenticate(self) -> bool:
        """
        Authenticate with Tradovate using access token.
        ...
        if not self.connected or not self.websocket:
            logger.error("UserSync: Cannot authenticate - not connected")
            return False
        ...
        try:
            ...
            logger.debug(f"UserSync: Sending auth message (id={msg_id})")
            await self.websocket.send(auth_message)
            
            # Wait for response
            response = await asyncio.wait_for(self.websocket.recv(), timeout=10.0)
            logger.debug(f"UserSync: Auth response: {response[:100]}...")
            
            # Check for successful auth (response should contain 's' for success)
            if '"s":200' in response or '"i":' + str(msg_id) in response:
                self.authenticated = True
                logger.info("✅ UserSync: Authenticated successfully")
                return True
            else:
                logger.error(f"❌ UserSync: Auth failed: {response[:200]}")
                return False
                
        except asyncio.TimeoutError:
            logger.error("❌ UserSync: Auth timeout")
            return False
        except Exception as e:
            logger.error(f"❌ UserSync: Auth error: {e}")
            return False
    
    async def subscribe_user_sync(self) -> bool:
        """
        ...
        if not self.authenticated:
            logger.error("UserSync: Cannot subscribe - not authenticated")
            return False
        ...
        try:
            ...
            logger.info(f"📡 UserSync: Subscribing to user {self.user_id}")
            await self.websocket.send(sync_message)
            
            # Wait for initial sync response (can be large)
            response = await asyncio.wait_for(self.websocket.recv(), timeout=30.0)
            logger.debug(f"UserSync: Sync response length: {len(response)}")
            
            # Parse initial sync data
            if response and len(response) > 0:
                self.subscribed = True
                self._parse_sync_response(response)
                logger.info("✅ UserSync: Subscribed to user sync")
                return True
            else:
                logger.error("❌ UserSync: Empty sync response")
                return False
                
        except asyncio.TimeoutError:
            logger.error("❌ UserSync: Sync request timeout")
            return False
        except Exception as e:
            logger.error(f"❌ UserSync: Sync error: {e}")
            return False
    
    def _parse_sync_response(self, response: str):
        """Parse the initial sync response and cache positions."""
        try:
            # Tradovate sends JSON with various entity types
            # Format varies, try to extract position data
            ...
            # May be a simple acknowledgment
            logger.debug(f"UserSync: Non-JSON response: {response[:100]}")
        except Exception as e:
            logger.debug(f"UserSync: Parse error: {e}")
    
    def _process_message(self, data: Any):
        """Process a single message from WebSocket."""
        import time
        
        if not isinstance(data, dict):
            return
        
        # Handle 'props' events (position/order updates)
        event_type = data.get('e')
        if event_type == 'props':
            entity_data = data.get('d', {})
            entity_type = entity_data.get('entityType')
            event_action = entity_data.get('eventType')  # Created, Updated, Deleted
            entity = entity_data.get('entity', {})
            
            if entity_type == 'position':
                self._handle_position_update(entity, event_action)
            elif entity_type == 'order':
                self._handle_order_update(entity, event_action)
            elif entity_type == 'cashBalance':
                self._handle_balance_update(entity, event_action)
        
        # Handle direct position data in sync response
        elif 'positions' in data:
            for pos in data.get('positions', []):
                self._handle_position_update(pos, 'Sync')
    
    def _handle_position_update(self, entity: Dict, event_action: str):
        """Handle position update event."""
        import time
        
        position_id = entity.get('id')
        if position_id:
            self.positions_cache[position_id] = entity
        
        self.last_update_time = time.time()
        
        logger.debug(f"📊 UserSync: Position {event_action}: {entity.get('contractId')} netPos={entity.get('netPos')}")
        
        if self.position_callback:
            try:
                self.position_callback(entity, event_action)
            except Exception as e:
                logger.error(f"UserSync: Position callback error: {e}")
    
    def _handle_order_update(self, entity: Dict, event_action: str):
        """Handle order update event."""
        logger.debug(f"📋 UserSync: Order {event_action}: {entity.get('id')}")
        
        if self.order_callback:
            try:
                self.order_callback(entity, event_action)
            except Exception as e:
                logger.error(f"UserSync: Order callback error: {e}")
    
    def _handle_balance_update(self, entity: Dict, event_action: str):
        """Handle balance update event."""
        logger.debug(f"💰 UserSync: Balance {event_action}: {entity.get('accountId')}")
        
        if self.balance_callback:
            try:
                self.balance_callback(entity, event_action)
            except Exception as e:
                logger.error(f"UserSync: Balance callback error: {e}")
    ...
    async def listen(self):
        ...
        if not self.subscribed:
            logger.error("UserSync: Cannot listen - not subscribed")
            return
        
        self._running = True
        logger.info("👂 UserSync: Starting message listener")
        ...
        try:
            while self._running and self.websocket:
                try:
                    ...
                    # Process the message
                    if message:
                        self._parse_sync_response(message)
                        
                except asyncio.TimeoutError:
                    # Send heartbeat to keep connection alive
                    try:
                        await self.websocket.ping()
                        logger.debug("UserSync: Heartbeat sent")
                    except:
                        logger.warning("UserSync: Heartbeat failed, reconnecting...")
                        break
                        
        except websockets.exceptions.ConnectionClosed as e:
            logger.warning(f"UserSync: Connection closed: {e}")
        except Exception as e:
            logger.error(f"UserSync: Listen error: {e}")
        finally:
            self._running = False
            self.connected = False
            self.authenticated = False
    ...
    async def disconnect(self):
        ...
        self.authenticated = False
        self.subscribed = False
        logger.info("🔌 UserSync: Disconnected")
    ...
    async def connect_and_subscribe(self) -> bool:
        """
        Convenience method: Connect, authenticate, and subscribe in one call.
        ...
        if not await self.subscribe_user_sync():
            await self.disconnect()
            return False
